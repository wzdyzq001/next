...
type FoodMainStatus = typeof FOOD_MAIN_STATUSES[number];

const FOOD_MAIN_STATUS_LABELS: Record<FoodMainStatus, string> = {
  pending_pay: '待支付',
  unused: '待使用',
  cancelled: '订单取消',
  completed: '交易完成',
...
const FOOD_SUB_STATUS_TO_MAIN: Record<string, FoodMainStatus> = {
  pending_accept: 'completed',
  merchant_accepted: 'completed',
  preparing: 'completed',
  merchant_preparing: 'completed',
  waiting_pickup: 'completed',
  waiting_rider: 'completed',
  delivering: 'completed',
  picked_up: 'completed',
  delivered: 'completed',
  redeemed: 'completed',
  delivery_pending_accept: 'completed',
  delivery_merchant_accepted: 'completed',
};

const FOOD_SELF_ORDER_SUB_STATUSES = [
  { value: 'pending_accept', label: '待商家接单' },
  { value: 'merchant_accepted', label: '商家已接单' },
  { value: 'preparing', label: '制作中' },
  { value: 'waiting_pickup', label: '待取餐' },
  { value: 'picked_up', label: '已取餐' },
];

const FOOD_DELIVERY_SUB_STATUSES = [
  { value: 'delivery_pending_accept', label: '待商家接单' },
  { value: 'delivery_merchant_accepted', label: '商家已接单' },
  { value: 'merchant_preparing', label: '商家备餐中' },
  { value: 'waiting_rider', label: '待骑手取餐' },
  { value: 'delivering', label: '配送中' },
  { value: 'delivered', label: '已送达' },
];

const FOOD_VOUCHER_SUB_STATUSES = [
  { value: 'redeemed', label: '已核销' },
];

function getDemoOrderMainStatus(order: DemoOrder): FoodMainStatus | string {
  if (order.category === 'food') {
    if (FOOD_MAIN_STATUSES.includes(order.orderStatus as FoodMainStatus)) {
      return order.orderStatus;
    }
    if (FOOD_SUB_STATUS_TO_MAIN[order.orderStatus]) {
      return FOOD_SUB_STATUS_TO_MAIN[order.orderStatus];
    }
  }
  return order.orderStatus;
}
...
function getDemoOrderMainStatusLabel(order: DemoOrder): string {
  ...
  if (order.category === 'food' && FOOD_MAIN_STATUS_LABELS[mainStatus as FoodMainStatus]) {
    return FOOD_MAIN_STATUS_LABELS[mainStatus as FoodMainStatus];
  }
  return order.orderStatusLabel;
}
...
const demoOrders: DemoOrder[] = [
  ...
  },
  {
    ...
    redeemMethod: 'delivery',
    redeemMethodLabel: '外卖',
    orderStatus: 'delivery_pending_accept',
  ...
  {
    id: 'food-delivery-merchant-accepted',
    category: 'food',
    categoryLabel: '餐饮',
    productType: 'group_voucher',
    productTypeLabel: '团购券',
    redeemMethod: 'delivery',
    redeemMethodLabel: '外卖',
    orderStatus: 'delivery_merchant_accepted',
    ...
    extension: {
      ...
      steps: [
        { label: '下单成功', state: 'done', time: '11:20' },
        { label: '商家已接单', state: 'done', time: '11:22' },
        { label: '商家备餐中', state: 'active', time: '11:23' },
        { label: '待骑手取餐', state: 'pending' },
        { label: '配送中', state: 'pending' },
        { label: '已送达', state: 'pending' },
      ],
    },
    actions: [],
    suggestions: ['还要多久', '可以加单吗', '可以取消吗'],
  },
  {
    ...
    redeemMethod: 'delivery',
    redeemMethodLabel: '外卖',
    orderStatus: 'merchant_preparing',
  ...
  },
  {
    ...
    extension: {
      ...
      estimatedTime: '预计15分钟送达',
      info: [
        { label: '配送骑手', value: '即将到达' },
      ],
      steps: [
        { label: '下单成功', state: 'done', time: '11:20' },
        { label: '商家已接单', state: 'done', time: '11:22' },
        { label: '商家备餐中', state: 'done', time: '11:25' },
        { label: '待骑手取餐', state: 'active', time: '11:32' },
        { label: '配送中', state: 'pending' },
        { label: '已送达', state: 'pending' },
      ],
    },
    actions: [],
    suggestions: ['骑手到哪了', '还有多久', '可以取消吗'],
  },
  {
  ...
  },
  {
    ...
    extension: {
      ...
      steps: [
        ...
        { label: '商家已接单', state: 'done', time: '11:22' },
        { label: '商家备餐中', state: 'done', time: '11:25' },
        { label: '待骑手取餐', state: 'done', time: '11:32' },
        { label: '配送中', state: 'done', time: '11:35' },
        { label: '已送达', state: 'done', time: '11:48' },
      ],
    },
    actions: [
  ...
  {
    id: 'food-pickup-merchant-accepted',
    category: 'food',
    categoryLabel: '餐饮',
    productType: 'group_voucher',
    productTypeLabel: '团购券',
    ...
    extension: {
      type: 'progress',
      title: '取餐进度',
      estimatedTime: '预计7分钟后可取',
      steps: [
        { label: '下单成功', state: 'done', time: '10:02' },
        { label: '商家已接单', state: 'done', time: '10:03' },
        { label: '制作中', state: 'active', time: '10:04' },
        { label: '待取餐', state: 'pending' },
  ...
  },
  {
    ...
    extension: {
      ...
      info: [
        { label: '取餐号', value: 'A088' },
        { label: '取餐时间', value: '10:12' },
      ],
      steps: [
        { label: '下单成功', state: 'done', time: '10:02' },
        { label: '商家已接单', state: 'done', time: '10:03' },
        { label: '制作中', state: 'done', time: '10:05' },
        { label: '待取餐', state: 'done', time: '10:09' },
        { label: '已取餐', state: 'done', time: '10:12' },
      ],
    },
    actions: [
      { label: '再来一单', type: 'primary' },
...
export function AiOrderCardDemo() {
  ...
  }, [activeTab, filterCategory, filterProductType, filterOrderStatus, filterSubStatus, expandedCompact]);
  ...
  const filteredOrders = demoOrders.filter(order => {
    ...
    if (filterOrderStatus !== 'all') {
      const mainStatus = getDemoOrderMainStatus(order);
      if (order.category === 'food') {
        if (mainStatus !== filterOrderStatus) return false;
        if (filterOrderStatus === 'completed' && filterSubStatus !== 'all') {
          if (order.orderStatus !== filterSubStatus) return false;
        }
      } else {
        if (order.orderStatus !== filterOrderStatus) return false;
      }
    }
        ...
        <div className="demo-filters">
          ...
          {filterCategory === 'food' ? (
            <>
              <div className="demo-filter-row">
                <div className="demo-filter-group">
                  <div className="demo-filter-label">主订单状态</div>
                  <div className="demo-filter-options">
                    {mainStatusOptions.map(opt => (
                      <button
                        ...
                        onClick={() => {
                          setFilterOrderStatus(opt.value);
              ...
              {filterOrderStatus === 'completed' && (
                <div className="demo-filter-row">
                  <div className="demo-filter-group">
                    <div className="demo-filter-label">子履约状态</div>
                    <div className="demo-filter-options">
                      <button
                        className={`demo-filter-chip ${filterSubStatus === 'all' ? 'active' : ''}`}
                        onClick={() => setFilterSubStatus('all')}
                      >
                        全部子状态
                      </button>
                      <span className="demo-filter-divider">自提</span>
                      {FOOD_SELF_ORDER_SUB_STATUSES.map(opt => (
                        <button
                          key={`self-${opt.value}`}
                          className={`demo-filter-chip ${filterSubStatus === opt.value ? 'active' : ''}`}
                          onClick={() => setFilterSubStatus(opt.value)}
                        >
                          {opt.label}
                        </button>
                      ))}
                      <span className="demo-filter-divider">配送</span>
                      {FOOD_DELIVERY_SUB_STATUSES.map(opt => (
                        <button
                      ...
                      <span className="demo-filter-divider">券码</span>
                      {FOOD_VOUCHER_SUB_STATUSES.map(opt => (
                        <button
                          key={`voucher-${opt.value}`}
                          className={`demo-filter-chip ${filterSubStatus === opt.value ? 'active' : ''}`}
                          onClick={() => setFilterSubStatus(opt.value)}
                        >
                          {opt.label}
            ...
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map((order, idx) => (
                <div key={`${order.id}-${idx}`} className="demo-card-item">
                  <div className="demo-card-label">{order.categoryLabel} · {order.productTypeLabel} · {getDemoOrderMainStatusLabel(order)}{order.category === 'food' && getDemoOrderMainStatus(order) === 'completed' && order.orderStatus !== 'completed' ? ` / ${order.orderStatusLabel}` : ''}</div>
